...
/** @var array $revenueByMonth    // [['month'=>'Jan 2025','gmv'=>120000,'orders'=>45], ...] */
/** @var array $categoryBreakdown // [['name'=>'Cleaning','count'=>80,'revenue'=>45000], ...] */
/** @var array $topVendors        // [['business_name'=>'...','orders'=>12,'revenue'=>80000,'rating'=>4.5], ...] */
/** @var array $userGrowth        // [['month'=>'Jan 2025','new_users'=>34,'providers'=>8], ...] */
/** @var array $orderStatuses     // ['completed'=>200,'pending'=>40,'cancelled'=>15] */
/** @var array $countyStats       // [['county'=>'Nairobi','orders'=>120,'revenue'=>95000], ...] */
/** @var float $totalGmv */
/** @var int   $totalOrders */
/** @var int   $totalUsers */
/** @var float $avgOrderValue */
/** @var float $gmvGrowthPct      // e.g. 15.4 */
...
.an-bar-fill  { height: 100%; border-radius: 4px; transition: width .5s ease; }
...
.an-funnel-bar { height: 28px; border-radius: var(--r-md); display: flex; align-items: center; padding: 0 10px; font-size: 11px; font-weight: 700; color: #fff; transition: width .5s ease; min-width: 40px; }
...
@media (max-width: 1000px) {
    .an-kpi-row  { grid-template-columns: repeat(2, minmax(0,1fr)); }
    .an-grid2    { grid-template-columns: 1fr; }
    .an-grid3    { grid-template-columns: 1fr 1fr; }
    .an-grid-eq  { grid-template-columns: 1fr; }
}
...
<div class="an-kpi-row">
    <div class="an-kpi v1">
        ...
        <div class="an-kpi-val"><?= anKes($totalGmv) ?></div>
...
<div class="an-grid2">
    ...
    <div class="hl-card">
        <div class="hl-card-head">
            <span class="hl-card-title">Order status</span>
        </div>
...
<!-- ── User growth chart + Category breakdown ── -->
<div class="an-grid2">
    <div class="hl-card">
        <div class="hl-card-head">
    ...
    <div class="hl-card">
        <div class="hl-card-head">
            <span class="hl-card-title">Category performance</span>
        </div>
        <div class="hl-card-body" style="padding-top:8px;">
            ...
            <div class="an-bar-row">
                ...
                <div class="an-bar-track">
                    <div class="an-bar-fill" style="width:<?= $barPct ?>%;background:<?= $catColors[$ci % count($catColors)] ?>;"></div>
                </div>
...
<div class="an-grid-eq">
    <div class="hl-card">
        ...
        <div class="hl-card-body" style="padding-top:8px;">
            ...
            <div class="an-bar-row">
                ...
                <div class="an-bar-track">
                    <div class="an-bar-fill" style="width:<?= $bPct ?>%;background:<?= $ctyColors[$ci % count($ctyColors)] ?>;"></div>
                </div>
                <div class="an-bar-meta">
    ...
    <div class="hl-card">
        ...
        <div class="hl-card-body">
            <?php
            /* Supply these from your controller: $funnel array */
            $funnel = $funnel ?? [
            ...
            <div class="an-funnel">
                ...
                <div class="an-funnel-row">
                    ...
                    <div class="an-funnel-bar" style="width:<?= $fPct ?>%;background:<?= $f['color'] ?>;">
                        <?= $fPct > 20 ? number_format($f['count']) : '' ?>
                    </div>
                    <?php if ($fPct <= 20): ?>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex;align-items:center;gap:9px;">
                            <div class="cust-avatar" style="background:<?= $vendorColors[$vi % count($vendorColors)] ?>;width:30px;height:30px;font-size:10px;flex-shrink:0;">
                    ...
                    <td>
                        <div style="display:flex;align-items:center;gap:7px;">
                            <div class="an-bar-track" style="width:70px;">
                                <div class="an-bar-fill" style="width:<?= min($share * 4, 100) ?>%;background:<?= $vendorColors[$vi % count($vendorColors)] ?>;"></div>
                            </div>
                            <span style="font-size:11px;color:var(--text3);"><?= $share ?>%</span>
                        </div>
                    </td>
                    <td>
                        <a href="<?= Url::to(['/admin/vendors/view', 'id' => $v['id'] ?? 0]) ?>" class="hl-card-link">View →</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var revenueMonths = <?= json_encode(array_column($revenueByMonth, 'month')) ?>;
    var gmvData       = <?= json_encode(array_column($revenueByMonth, 'gmv')) ?>;
    var ordersData    = <?= json_encode(array_column($revenueByMonth, 'orders')) ?>;

    new ApexCharts(document.querySelector('#anRevenueChart'), {
        series: [
            { name: 'GMV (KES)', type: 'area', data: gmvData },
            { name: 'Orders',    type: 'line', data: ordersData }
        ],
        chart: { height: 280, toolbar: { show: false }, background: 'transparent', zoom: { enabled: false } },
        ...
        yaxis: [
            { labels: { style: { colors: tickColor, fontSize: '10px' }, formatter: v => (v/1000).toFixed(0)+'K' } },
            { opposite: true, labels: { style: { colors: tickColor, fontSize: '10px' } } }
        ],
        legend: { position: 'top', fontSize: '11px', fontFamily: 'Plus Jakarta Sans, sans-serif',
    ...
    var statusLabels = <?= json_encode(array_map('ucfirst', array_keys($orderStatuses))) ?>;
    var statusVals   = <?= json_encode(array_values($orderStatuses)) ?>;
    ...
    /* ── 3. User + provider growth bar ── */
    var growthMonths    = <?= json_encode(array_column($userGrowth, 'month')) ?>;
    var newUsersData    = <?= json_encode(array_column($userGrowth, 'new_users')) ?>;
    var newProvidersData= <?= json_encode(array_column($userGrowth, 'providers')) ?>;

    new ApexCharts(document.querySelector('#anUserGrowthChart'), {
        series: [
            { name: 'New users',     data: newUsersData },
            { name: 'New providers', data: newProvidersData }
        ],
        chart: { type: 'bar', height: 240, toolbar: { show: false }, background: 'transparent', zoom: { enabled: false }, stacked: false },
        colors: ['#6C5CE7','#00B894'],
    ...
    /* ── Period tab UI (wire to your own AJAX/reload logic) ── */
    document.querySelectorAll('.an-tab').forEach(function (tab) {
        tab.addEventListener('click', function () {
            ...
            /* TODO: reload data via AJAX or page reload with ?period=X */
            /* Example: window.location.href = '<?= Url::to(['/admin/analytics']) ?>?period=' + tab.dataset.period; */
        });
    });
});
</script>